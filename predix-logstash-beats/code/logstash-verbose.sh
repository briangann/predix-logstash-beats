...
nohup ${HOME}/filebeat-1.2.3-x86_64/filebeat -v -e -c ${HOME}/filebeat.yml 2>&1 > ${HOME}/logs/filebeat.log &
#
# now chisel
#
echo "Starting up chisel-server!"
nohup ${HOME}/chisel-bin/chisel_linux_amd64 server -v --port ${PORT} 2>&1 > ${HOME}/logs/chisel-server.log &
...
${HOME}/logstash-2.3.4/bin/logstash --verbose -f ${HOME}/logstash/logstash-beats.conf -l ${HOME}/logs/logstash.log 2> ${HOME}/logs/logstash-stderr.log
